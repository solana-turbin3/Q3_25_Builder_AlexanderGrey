...
mod tests {
    ...
    use solana_program::{pubkey::Pubkey, system_instruction::transfer, system_program, hash::hash};
    use solana_sdk::{
    ...
    fn transfer_sol() {
        ...
        let to_pubkey = Pubkey::from_str("7DssYoo3p4FH5RfRvoin9XgdRXyUhmjLDoVRWaZHb4N9").unwrap();
    ...
    #[test]
    fn convert_base58_key() {
        let base58_key = "5DY7b42frspQq2yGBJCCTVx1yRBVoncwRnKPyrBxtZyiXwRETuo8zqV4ktaEWcdfr3NncxTBU3mYd41cG7sijm8Z";
        let wallet_bytes = bs58::decode(base58_key).into_vec().unwrap();
        println!("Converted wallet bytes: {:?}", wallet_bytes);
        
        let keypair = Keypair::from_bytes(&wallet_bytes).unwrap();
